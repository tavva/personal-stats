...
def home():
    fitbit = {
        'has_access_token': f"fitbit_access_token:{flask.g.user_id}" in db,
        'has_refresh_token': f"fitbit_refresh_token:{flask.g.user_id}" in db,
    }
...
def handle_redirect_from_fitbit():
    ...
    db[f"fitbit_access_token:{flask.g.user_id}"] = data['access_token']
    db[f"fitbit_refresh_token:{flask.g.user_id}"] = data['refresh_token']
...
def request_authorisation():
    ...
    code_challenge = base64.urlsafe_b64encode(
        hashlib.sha256(
            code_verifier.encode("utf-8")).digest()).decode("utf-8")[:-1]
    ...
    authorization_url = "https://www.fitbit.com/oauth2/authorize?" + urlencode(
        params)
